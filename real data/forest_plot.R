...
get_sievetl_plugin_transfer_results <- function(covariate_name,
  ...
  if(display_name == 'Cox-SieveTL') {
    infer_file <- '/Users/yuxisong/Library/CloudStorage/Box-Box/Cox-SieveTL/simulation/results/new_icb_response_result/results_allTMB_new_hessian_lasso/realdata_transfer_hessian_inference/all_targets_beta_wald_ci.csv'
  } else {
...
make_forest_panel_all_methods <- function(
  ...
  method_levels <- c("CoxPH Target", "Cox-SieveTL", "TransCox", "DiscreteKL")
...
forest_all_methods_plot <- make_forest_panel_all_methods(
  df = all_methods_df,
